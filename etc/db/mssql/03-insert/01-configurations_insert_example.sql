...
-- DATASOURCE: MSSQL
insert into TB_CONFIGURATIONS (CONF_KEY,CONF_VALUE) values ('metamac.security.datasource.url','jdbc:sqlserver://FILL_ME_WITH_HOST:FILL_ME_WITH_PORT;databaseName=DATABASE_NAME');
...
insert into TB_CONFIGURATIONS (CONF_KEY,CONF_VALUE) values ('metamac.security.datasource.driver_name','com.microsoft.sqlserver.jdbc.SQLServerDriver');
insert into TB_CONFIGURATIONS (CONF_KEY,CONF_VALUE) values ('metamac.security.datasource.query.attributes','SELECT usr.username as username, ''ACL'' as acl_name, ISNULL(app.code, '''') + ''#'' + ISNULL(rol.code, '''') + ''#'' + ISNULL(operations.code, '''') as acl_value
        FROM TB_ACCESS acc
